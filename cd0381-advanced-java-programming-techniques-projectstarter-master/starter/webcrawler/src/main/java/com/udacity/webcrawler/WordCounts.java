...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    ...
    //Sorted so that the more frequent words come first
    WordCountComparator wordCountComparator = new WordCountComparator();
    return wordCounts.entrySet().stream().sorted(wordCountComparator).limit(Math.min(popularWordCount, wordCounts.size()))
