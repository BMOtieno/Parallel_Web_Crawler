...
final class ProfilerImpl implements Profiler {
  ...
  private boolean checkProfiledClass(Class<?> klass){
    ArrayList<Method> profiledMethod = new ArrayList<>(Arrays.asList(klass.getDeclaredMethods()));

    if(profiledMethod.isEmpty()){
      return false;
    }
    return profiledMethod.stream().anyMatch(myVariable -> myVariable.getAnnotation(Profiled.class) != null);
  }
  ...
  public <T> T wrap(Class<T> klass, T delegate) {
    ...
    if(!checkProfiledClass(klass)){
      throw new IllegalArgumentException(klass.getName() + "lacks profiled methods");
    }
